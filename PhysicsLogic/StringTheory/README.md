...
- `AdS3CFT2.lean`: Section-22 interfaces for D1-D5/AdS3-CFT2 moduli and decoupling, AdS3 WZW/spectral-flow spectra, and mixed `(NS,NS)`/`(R,R)` flux deformation packages including long-string threshold/discretization transition, pulsating-threshold pole relation (`n=k/4`, `Delta=k/2` in NSNS), RR-deformation SFT recursion, explicit `W^(1)⊗W^(1)` two-string-bracket structure, four-point mass-shift extraction, finite-`k` WZW reduction constraints, and finite-`k` WZW OPE-constant normalization data.
- `AdS3CFTBridge.lean`: cross-lane interfaces tying Section-22 D1-D5 conformal-manifold/symmetric-orbifold assumptions and AdS3 WZW/spectral-flow/worldsheet/GSO/supersymmetric-NSNS mass-shell/mixed-flux/long-string-transition/pulsating-threshold/SFT-mass-shift/finite-`k`-reduction/OPE-constant/two-string-bracket packages to shared QFT 2D-CFT modules.

...
- `AdS3CFT2.lean`: Section-22 interfaces for D1-D5/AdS3-CFT2 moduli and decoupling, AdS3 WZW/spectral-flow spectra, and mixed `(NS,NS)`/`(R,R)` flux deformation packages including explicit `mu`/`k` definition (`mu=g_B Q5/K5`, `k=K5`), long-string threshold/discretization transition, pulsating turning-point relation (`r0` maximal, `dot r=0`, `Delta(r0)`), pulsating integral-quantization unit, compositional Bohr-Sommerfeld package, compositional reconstruction package for the small-`mu` pulsating spectrum from `mu`/`k` + Bohr-Sommerfeld units, pulsating-threshold pole relation (`n=k/4`, `Delta=k/2` in NSNS), RR-deformation SFT recursion, explicit `W^(1)⊗W^(1)` two-string-bracket structure, second-order RR-SFT compositional package tied to finite-`k` bracket vanishing, four-point mass-shift extraction, RR-spectrum-from-second-order package, pulsating/mass-shift consistency package (matching semiclassical `Delta(mu)` with RR `delta m^2` relation), finite-`k` WZW reduction constraints, finite-`k` WZW OPE-constant normalization data, compositional RR-spectrum correction package assembling SFT/bracket/reduction/OPE units into the mass-shift law, and end-to-end semiclassical/quantum match packages via both general RR-compositional and second-order-RR-SFT-derived routes.
- `AdS3CFTBridge.lean`: cross-lane interfaces tying Section-22 D1-D5 conformal-manifold/symmetric-orbifold assumptions and AdS3 WZW/spectral-flow/worldsheet/GSO/supersymmetric-NSNS mass-shell/mixed-flux/mu-k-definition/long-string-transition/pulsating-turning-point/pulsating-integral-quantization/pulsating-bohr-sommerfeld/pulsating-compositional-reconstruction/pulsating-threshold/SFT-mass-shift/pulsating-mass-shift-consistency/finite-`k`-reduction/OPE-constant/two-string-bracket/sft-second-order-compositional/RR-spectrum-compositional-correction/RR-spectrum-from-second-order/semiclassical-quantum-match/semiclassical-quantum-match-from-second-order packages to shared QFT 2D-CFT modules.

...
def EventHorizon (consts : GRConstants) (bh : BlackHole consts)
  ...
  {x | ¬∃ (γ : Curve), NullGeodesic bh.connection γ ∧
       γ 0 = x ∧ ∃ (t : TimeScale), t > 0 ∧ γ t ∈ asymp.futureNullInfinity}
...
def TrappedSurface (_metric : SpacetimeMetric) (S : Set SpaceTimePoint) : Prop :=
  ∀ p ∈ S, ∃ (θ_out θ_in : SurfaceGravityScale), θ_out < 0 ∧ θ_in < 0

/-- Apparent horizon: outermost trapped surface at given time -/
def ApparentHorizon (consts : GRConstants) (bh : BlackHole consts) (t : TimeScale) :
    Set SpaceTimePoint :=
...
structure BlackHoleThermodynamics (consts : GRConstants) (bh : BlackHole consts) where
  ...
  surfaceGravity : (ξ : SpaceTimePoint → Fin 4 → ScalingDimension) →
    TimelikeKilling bh.connection ξ → SurfaceGravityScale
  /-- Bekenstein-Hawking entropy: S_BH = (k_B c³/4ℏG) A -/
  bekensteinHawkingEntropy : EntropyMeasure
  /-- Hawking temperature: T_H = ℏκ/(2πk_B c) -/
  hawkingTemperature : SurfaceGravityScale → TemperatureScale  -- function of κ
  /-- Horizon area as a function of time -/
  horizonArea : TimeScale → AreaScale
  ...
  first_law : ∀ (ξ : SpaceTimePoint → Fin 4 → ScalingDimension)
      (h : TimelikeKilling bh.connection ξ),
  ...
  hawking_area_theorem : ∀ (T : TensorField 4 4),
    ...
    ∀ (t₁ t₂ : TimeScale), t₁ ≤ t₂ → horizonArea t₁ ≤ horizonArea t₂

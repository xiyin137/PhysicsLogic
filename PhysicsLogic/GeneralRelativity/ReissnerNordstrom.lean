...
noncomputable def reissnerNordstromMetricTensor
    (consts : GRConstants) (M : MassScale) (Q : ChargeScale)
    (x : SpaceTimePoint) (mu nu : Fin 4) : ℝ :=
  let r := Real.sqrt ((x 1)^2 + (x 2)^2 + (x 3)^2)
  let delta :=
    1 - 2 * (consts.G : ℝ) * M.value / (r * (consts.c : ℝ)^2) +
      (consts.G : ℝ) * Q.value^2 / (r^2 * (consts.c : ℝ)^4)
  match mu, nu with
  | 0, 0 => -delta * (consts.c : ℝ)^2
  | 1, 1 => 1 / delta
  | 2, 2 => r^2
...
noncomputable def reissnerNordstromMetricDeterminant
  ...
  -(consts.c : ℝ)^2 * (Real.sqrt ((x 1)^2 + (x 2)^2 + (x 3)^2))^4 * (Real.sin (x 2))^2

/-- Well-formedness package for the abstract RN metric data. -/
def ReissnerNordstromMetricWellFormed (consts : GRConstants) (M : MassScale) (Q : ChargeScale) : Prop :=
  (∀ x mu nu,
...
noncomputable def reissnerNordstromMetric (consts : GRConstants) (M : MassScale) (Q : ChargeScale)
    (h_phys :
...
noncomputable def rnInnerHorizon (consts : GRConstants) (M : MassScale) (Q : ChargeScale) :
    LengthScale :=
  ((consts.G : ℝ) * M.value / (consts.c : ℝ)^2 -
    Real.sqrt
      (((consts.G : ℝ) * M.value / (consts.c : ℝ)^2)^2 -
        (consts.G : ℝ) * Q.value^2 / (consts.c : ℝ)^4) : ℝ)

/-- Outer (event) horizon -/
noncomputable def rnOuterHorizon (consts : GRConstants) (M : MassScale) (Q : ChargeScale) :
    LengthScale :=
  ((consts.G : ℝ) * M.value / (consts.c : ℝ)^2 +
    Real.sqrt
      (((consts.G : ℝ) * M.value / (consts.c : ℝ)^2)^2 -
...
structure RNTheory (consts : GRConstants) (M : MassScale) (Q : ChargeScale) where
  ...
  electricField : LengthScale → ElectricFieldScale  -- as function of radius
  /-- Electric field formula: E = Q/(4πε₀r²) -/
  electric_field_formula :
    ∀ (r : LengthScale), r > 0 → electricField r = Q / (4 * Real.pi * r.value ^ (2 : ℕ))
...
def IsExtremalRN (consts : GRConstants) (params : RNBlackHoleParams consts) : Prop :=
  params.charge.value ^ (2 : ℕ) =
    (((consts.G : ℝ) * params.mass / (consts.c : ℝ)^2) ^ (2 : ℕ))

/-- Structure for RN singularity and horizon analysis -/
structure RNSingularityTheory (consts : GRConstants)
    (M : MassScale) (Q : ChargeScale) (rn : RNTheory consts M Q) where
  /-- Naked singularity if Q² > (GM/c²)² (violates weak cosmic censorship?) -/
  naked_singularity_if_overcharged :
    Q.value^2 > (((consts.G : ℝ) * M.value / (consts.c : ℝ)^2) ^ (2 : ℕ)) →
